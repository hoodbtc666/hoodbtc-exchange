...
export default function Home() {
  ...
  const sdkChains = useMemo(() => {
    return (Array.isArray(orderlyChains) ? orderlyChains : []).filter((chain: any) => chain?.network === "mainnet" || !chain?.network);
  }, [orderlyChains]);
    ...
    <main className={dense ? "hoodbtcTradingShell denseMode" : "hoodbtcTradingShell"}>
      <header className="proTopbar">
        ...
        <nav className="proNav" aria-label="Trading navigation">
          <span className="active">Perpetuals</span>
          <span className="liquidityText">Shared Liquidity</span>
        </nav>

        <div className="proRight">
          <div className="networkControl">
            <button className="networkButton" type="button" onClick={() => { setNetworkOpen((v) => !v); setMenuOpen(false); }} aria-expanded={networkOpen}>
            ...
            {networkOpen && (
              <div className="networkMenu">
                ...
                <div className="supportedGrid">
                  {sdkChains.map((chain: any) => (
                    <div className="supportedChain" key={String(chain?.id ?? chain?.chainId ?? chain?.label)}>
                      {chain?.icon ? <img src={chain.icon} alt="" /> : <i />}
                      <span>{chain?.label || chain?.name || chain?.id}</span>
                    </div>
                  ))}
                </div>
          ...
          <button className="connectButton" type="button" onClick={handleConnect} disabled={connecting} title={walletAddress || "Connect wallet"}>
            {connecting ? "Connecting…" : shortAddress(walletAddress)}
          </button>

          <button className="terminalMode" type="button" onClick={() => setDense((v) => !v)} title="Toggle terminal density">
            {dense ? "Pro" : "Comfort"} ⇄
          </button>

          <div className="moreControl">
            <button className="menuButton" type="button" aria-label="Open menu" onClick={() => { setMenuOpen((v) => !v); setNetworkOpen(false); }}>☰</button>
            {menuOpen && (
              <div className="terminalMenu">
                <div className="menuBrand"><img src={HOODBTC_LOGO} alt="" /><div><b>HOODBTC</b><small>Trading Terminal</small></div></div>
                <a href="https://hoodbtc.com" target="_blank" rel="noreferrer">HOODBTC Home <span>↗</span></a>
                <div className="terminalInfo"><span>Environment</span><b>Mainnet</b></div>
                <div className="terminalInfo"><span>Custody</span><b>Self-custody</b></div>
                <div className="terminalInfo"><span>Liquidity</span><b>Shared</b></div>
                <div className="terminalInfo"><span>Default Market</span><b>BTC-PERP</b></div>
              </div>
            )}
          </div>
        </div>
